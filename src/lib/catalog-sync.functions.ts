...
export const syncCatalogFromSheet = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: logIns } = await supabaseAdmin
      .from("sync_logs")
      .insert({ source: "google_sheets_catalog", status: "running", message: "Старт синхронизации" })
      .select("id")
      .single();
    ...
    try {
      ...
      // ---- 2. Warehouses (из колонки F — Локация) -------------------------
      ...
      // ---- 3. Aggregate products & stock from rows -------------------------
      type ProdKey = string; // `${brandId}::${sku}`
      ...
      >();
      const stockAgg = new Map<string, number>(); // `${prodKey}::${whId}` -> qty
      let skipped = 0;
      ...
      // ---- 4. Upsert products (по brand_id+sku) ----------------------------
      // products table has no unique constraint we can rely on for onConflict — fetch then split insert/update
      ...
      const { data: existingProds } = await supabaseAdmin
        ...
        .in("sku", skuList);
      const prodIdMap = new Map<string, string>(); // key -> product.id
      ...
      for (const [key, p] of products) {
        ...
        if (ex) {
          ...
          toUpdate.push({
            id: ex.id,
            name: p.name,
            base_price: p.base_price || ex.base_price,
            category: p.category,
          });
        } else {
      ...
      // Updates (only when name/price changed) — batched per-row
      let updated = 0;
      ...
      // ---- 5. Replace stock for synced products ----------------------------
      const allProductIds = [...prodIdMap.values()];
      let stockReplaced = 0;
      if (allProductIds.length) {
        // delete existing stock rows for these products
      ...
      if (logId) {
        await supabaseAdmin
          ...
          .update({
            ...
            rows_failed: skipped,
            message: `OK: ${inserted} новых, ${updated} обновлено, ${stockReplaced} остатков`,
            details: summary,
          })
          .eq("id", logId);
      }
